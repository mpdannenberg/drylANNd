...
for j = [1:25 27:n]
    ...
    C = Ameriflux_8day(setdiff(1:n, j)); 
    V = Ameriflux_8day(j);
...
%     Yvs = repmat(std(Yc, 0, 2, 'omitnan'), 1, size(Yv, 2));
    ...
    Ameriflux_8day(j).GPP_DrylANNd = Yv_mean(1,:)';
    Ameriflux_8day(j).NEE_DrylANNd = Yv_mean(2,:)';
    Ameriflux_8day(j).ET_DrylANNd = Yv_mean(3,:)';
    ...
    print('-dtiff','-f1','-r300',['./output/validation/8day/DrylANNd_8day_timeseries_',V.Site,'.tif'])
    close all;
...
save('output/validation/8day/DrylANNd_Ameriflux_validation.mat', 'Ameriflux_8day', '-v7.3');
