...
%     LSWI3_f = filtering_outliers(LSWI3, doy, 'lwl',3, 'uwl',3, 'thres',0.1*range(LSWI3), 'cycles',2, 'winsize',15);
    ...
    Ameriflux(i).NDVI(ia) = NDVI(ib);
    Ameriflux(i).EVI(ia) = EVI(ib); 
    Ameriflux(i).NIRv(ia) = NIRv(ib);
    Ameriflux(i).kNDVI(ia) = kNDVI(ib);
    Ameriflux(i).LSWI1(ia) = LSWI1(ib);
    Ameriflux(i).LSWI2(ia) = LSWI2(ib);
    Ameriflux(i).LSWI3(ia) = LSWI3(ib);
