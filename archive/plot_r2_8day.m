...
[~, I] = sort(lc);
Ameriflux_8day = Ameriflux_8day(I);
lc = lc(I);
xlbs = xlbs(I);
...
for i = 1:n
    ...
    scatter(Ameriflux_8day(i).GPP, Ameriflux_8day(i).GPP_DrylANNd, 5, clr(ia,:), 'filled')
end
...
for i = 1:n
    ...
    y = Ameriflux_8day(i).GPP;
    yhat = Ameriflux_8day(i).GPP_DrylANNd;
    ib = ~isnan(yhat);
    ...
    r = corr(y(ib), Ameriflux_8day(i).MOD17_GPP(ib), 'rows','pairwise')^2;
    plot(i, r, '+', 'Color', [0.6 0.6 0.6], 'LineWidth',1.2)
end

% ENF overall
C = Ameriflux_8day(strcmp(lc, ulc(1)));
...
% GRS overall
C = Ameriflux_8day(strcmp(lc, ulc(2)));
y = extractfield(C, 'GPP');
yhat1 = extractfield(C, 'GPP_DrylANNd');
yhat2 = extractfield(C, 'MOD17_GPP');
ib = ~isnan(yhat1);
...
% SAV overall
C = Ameriflux_8day(strcmp(lc, ulc(3)));
y = extractfield(C, 'GPP');
yhat1 = extractfield(C, 'GPP_DrylANNd');
yhat2 = extractfield(C, 'MOD17_GPP');
ib = ~isnan(yhat1);
...
% SHB overall
C = Ameriflux_8day(strcmp(lc, ulc(4)));
y = extractfield(C, 'GPP');
yhat1 = extractfield(C, 'GPP_DrylANNd');
yhat2 = extractfield(C, 'MOD17_GPP');
ib = ~isnan(yhat1);
...
for i = 1:n
    ia = find(strcmp(lc{i}, ulc));
    scatter(Ameriflux_8day(i).NEE, Ameriflux_8day(i).NEE_DrylANNd, 5, clr(ia,:), 'filled')
end
set(gca, 'XLim',ylim, 'YLim',ylim, 'TickDir','out', 'TickLength',[0.02 0.])
box off;
hold off;
...
for i = 1:n
    ...
    y = Ameriflux_8day(i).NEE;
    yhat = Ameriflux_8day(i).NEE_DrylANNd;
    ib = ~isnan(yhat);
    r = corr(y(ib), yhat(ib), 'rows','pairwise')^2;
...
for i = 1:n
    ia = find(strcmp(lc{i}, ulc));
    scatter(Ameriflux_8day(i).ET, Ameriflux_8day(i).ET_DrylANNd, 5, clr(ia,:), 'filled')
end
set(gca, 'XLim',ylim, 'YLim',ylim, 'TickDir','out', 'TickLength',[0.02 0.])
box off;
hold off;
...
for i = 1:n
    ...
    y = Ameriflux_8day(i).ET;
    yhat = Ameriflux_8day(i).ET_DrylANNd;
    ib = ~isnan(yhat);
    r = corr(y(ib), yhat(ib), 'rows','pairwise')^2;
    ...
    r = corr(y(ib), Ameriflux_8day(i).MOD16_ET(ib), 'rows','pairwise')^2;
    plot(i, r, '+', 'Color', [0.6 0.6 0.6], 'LineWidth',1.2)
end
...
print('-dtiff','-f1','-r300','./output/validation/8day/overall-site-r2.tif')
close all;
